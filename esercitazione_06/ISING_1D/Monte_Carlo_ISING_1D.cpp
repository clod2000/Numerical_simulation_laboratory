...
int main(int argc , char** argv) {
   ...
   if(argc != 2){
        cerr<<"program usage: if <0> run simulation for various time,  <1> run for static time "<<endl;
        return -1; 
    }
  ...
  if(var == 0) {
    ...
    for(int i{}; i<16;++i){
      ...
      for( int i{}; i< 100000; ++i)   //Equilibration
       // Move(metro);
    ...
    for( int i{}; i< 10000; ++i)   //Equilibration
      Move(metro);
...
//initial configuration
  ...
  if(restart) {
    cout<<"Starting from previous configuration"<<endl;
    ifstream Readconf("config.final");
      for (int i=0; i<nspin; ++i)
        Readconf>>s[i];   
  }else{
    cout<<"Preparing random configuration"<<endl;
    for (int i=0; i<nspin; ++i){
...
// INCLUDE YOUR CODE HERE
    ...
    //new
      ...
      //test
      p = min(1. , exp( beta*(energy_old-energy_new)) );
      if(p >= rnd.Rannyu() ){
        //Updating
        s[o] = sm;
        accepted = accepted + 1; 
...
//cycle over spins
  for (int i=0; i<nspin; ++i)
  {
    u += -J * s[i] * s[Pbc(i+1)] - 0.5 * h * (s[i] + s[Pbc(i+1)]); //internal energy
