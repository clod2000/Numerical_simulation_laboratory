...
const int ncity = 34;
const int nind = 1000;  // individual for each population
const int numgen = 200; // number of generations
const int type_sim = 0; // if 0 circle, else square
const double pm = 0.15;  // mutation probability
const double pc = 0.9;  // crossover probability
const double p_sel = 3; // selection operator exponent
...
public:
    // constructors
    Population(arma::mat pos, int nindividuals) : m_L(nindividuals)
    {
        m_rnd.StartGen();
    ...
    Population(Population &P) : m_L(P.Get_nrows())
    {
        m_rnd.StartGen();
...

{
    ...
    /*
        ...
        vector<mat> copiator(m);  // vettore su cui copiare temporaneamente i dati
        ...
        for(int i{}; i < m; ++i){    // inserisco gli elementi rimossi nella nuova posizione
            lcopy.emplace(lcopy.begin() + index[i] , copiator[i]);
           // lcopy[1+index[i]]=copiator[i];
        }
    ...
    */
    ...
    for (int i{}; i < m; ++i) // eliminate the elements ranging from a to a+m
        lcopy.erase(lcopy.begin() + a);
...
void Population::Mutation3(int nrow)
{
    ...
    int a = m_rnd.Rannyu(1., med - m);      //  generate the starting position of the m elements
    int b = m_rnd.Rannyu(med, 2 * med - m); // generate the starting position of the other m elements
...
void Population::Save_L_mean(int j)
{
    ofstream outL;
    outL.open("output.L_means.dat", ios::app);
...
{
    return int(m_pop.n_rows * pow(m_rnd.Rannyu(), p_sel)) + 1;
}
...
void Population::Sort_for_L()
{
    // cout<<"start"<<endl;
    Calculate_L();
    arma::field<mat> fcopy(m_pop.n_rows, m_pop.n_cols); // create copy field
    vector<double> vcopy;
    fcopy = m_pop;

    int index;
    // cout<<"starting for cycle"<<endl;
    for (unsigned int i{}; i < m_pop.n_rows; ++i)
    { // cerco il valore minore di L
        index = selection_best_L();
        vcopy.push_back(m_L[index]);
        // cout<<index<<endl;

        for (unsigned int j{}; j < m_pop.n_cols; ++j) // put the best individual in position i
            m_pop(i, j) = fcopy(index, j);
        m_L[index] += 1E5; // this is so that at the next round of the cycle the best element is no longer taken

    }
    m_L = vcopy; // reset m_L in the correct order
}
...
void Population::Crossover(int rw1, int rw2)
{
   ...
   int row1 = rw1;
    int row2 = rw2;
    if(m_rnd.Rannyu()<0.5 || row1== 0)               // 50% probability to save the parent
        row1 += Get_nrows() / 2;
    if(m_rnd.Rannyu()<0.5)     
