...
//Read input informations
  ...
  ptail = 32.*M_PI*rho*( 1./(9.*pow(rcut,9)) - 1./( 6.*pow(rcut,3)  ) )*(double)npart/vol;
...
//Print initial values for measured properties
  ...
  cout << "Initial pression         = " << walker[ip] << endl;
...
//update the histogram of g(r)
      iterg =ig + int(dr/bin_size);
     // cout<<"dr = "<<dr<<"\tbin_size = "<<bin_size<<endl;
     // cout<<"int = "<<int(dr/bin_size)<<"\n"<<endl;
      if (iterg <= ig + nbins)
        walker[iterg] += 2;
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12, wde=20, pde=10;
    ...
    stima_pres = blk_av[ip]/blk_norm + ptail;  //Pression
  ...
  //stima g(r)
    ...
    for(int i = ig; i < m_props; ++i){
      ...
      r = (i +1-ig)*bin_size;
      r_norm =  rho*(double)m_part*4./3.*M_PI*( pow( (r +bin_size), 3 ) - pow(r,3) );       
      //cout<<r_norm<<endl;
...
//Total energy
  Etot << setw(wd) << iblk <<  setw(wde)<< setprecision(pde) << stima_etot << setw(wde)<<setprecision(pde) << glob_av[ie]/(double)iblk << setw(wde) << setprecision(pde) << err_etot << endl;
//Temperature
  Temp << setw(wd) << iblk <<  setw(wd) << stima_temp << setw(wd) << glob_av[it]/(double)iblk << setw(wd) << err_temp << endl;
//Pression
  Press << setw(wd) << iblk <<  setw(wd) << stima_pres/(double)iblk << setw(wd) << glob_av[ip]/(double)iblk << setw(wd) << err_press << endl;
...
void Print_g_r() {
  ofstream outg, outp;
  outg.open("output_g.dat");
  outp.open("output_parameters_for_g.dat");

  outp<<box<<'\t'<<nbins<<'\t'<<bin_size<<endl;

  for(int i = ig; i < m_props; ++i)
    outg<<i*bin_size<<setw(15)<<glob_av[i]/(double)nblk<<setw(15)<<err_g[i]<< endl;
  outg.close();
